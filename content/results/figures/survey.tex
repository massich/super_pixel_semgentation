...
%    \draw (\cAngle:{\cZoom*1.4}) node[align=center] {{\color{blue}\nreward $\%$} \\ {\color{red}\n} };
  ...
  \foreach \numElm/\contadorQueNoSeCalcular/\name [count=\ni] in \putaa
  {
    ...
    \draw (\initialAngle:{\cZoom*1.1}) -- (\initialAngle:{\cZoom*1.1});
    \draw [ |<->|,>=latex] (\initialAngle:\l) arc (\initialAngle:\finalAngle:\l) ;    									 
    \pgfmathsetmacro\r  {\cZoom*1.1+.7pt}
  ...
  \node [anchor=north] at (0, \cZoom*-1.25){
    \begin{tikzpicture}
      \begin{customlegend}[ 
        legend columns=-1,
        legend style={ draw=none, column sep=1em},
        legend entries={Dataset s/m/l, manual AOV\cite{gerard2013}, AOV, obtained AOV}
        ]
