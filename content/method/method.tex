...
and (3) re-mapping or decoding the labels obtained from the previous stage to produce the final delineation. 
...
Let $\mathcal{W}$ be all the possible labelling configurations of the set $\mathcal{S}$ given $\mathcal{L}$; and, let $U(\cdot)$ be a cost function encoding how good is a labelling configuration $\omega \in \mathcal{W}$ based on the appearance of the elements in $\mathcal{S}$, their inner relation and some designed constraints.
...
The nature of $U(\cdot)$ and $\mathcal{W}$ dictates which minimization strategies should be applied to find $\hat{\omega}$ since that not every strategy is suitable or desirable.
...
\Cref{fig:methodterms} illustrates the terms and elements relating the framework outline, presented in \cref{fig:method}, with its formulation in \cref{eq:labelingeq}.
...
For this work $\mathcal{S}$ is chosen to be a super-pixels representation of the image~\cite{achanta2012slic}. 
...
\Cref{fig:methodTerms:problem} shows a \ac{bus} image example and its associated super-pixels representation $\mathcal{S}$ coloured according to the image's \ac{gt}.
...
To do so, using the strategy here proposed, it is required to define: a data term, a pairwise term, and a proper minimization methodology.
...
The idea is to generate image or data model for each class from training samples, and let $D(\cdot)$ be a distance or goodness measure reflecting the likelihood of $s$ to belong to class $l$.
...
The final manuscript has a section dedicated to describe and analyse the features designed for \ac{bus} images based on \cref{fig:lesions}. Other analysis regarding the \ac{ml} stage are out of the study here presented.
...
In this manner the regularization term can be seen as a post-processing or denoising stage since some sites will flip their labelling if the cost of producing an edge is larger than the cost of adopting the neighbour's label. 
...
The minimization strategy used in this work is \ac{gc}. 
...
Since, when \ac{gc} is applicable, it allows to rapidly find a strong local minima guaranteeing that no other minimum with lower energy can be found~\cite{delong2012fast}. 
\ac{gc} is applicable if, and only if, the pairwise term favours coherent labelling configurations and penalizes labelling configurations where neighbour labels differs. 
