...
Despite the fact that construction details of $D(\cdot)$ are out of the scope of this report, the rest of this~\cref{sec:method:dataTerm} summarizes the process whereas further details can be found at~\citeauthor{sikphd}.
...
\Cref{fig:features} relates the \ac{birads} lexicon for describing \ac{bus} images~\cite{biradsus}, with the features designed to build $D(\cdot)$.
...
\Cref{fig:features} relates the lexicon widely used by the radiologists for describing \ac{bus} images~\cite{biradsus}, with the features designed to build $D(\cdot)$.
