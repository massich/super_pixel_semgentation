...
\Cref{fig:features} is a three-parts illustration to graphically summarize the visual cues that can be found in \ac{bus} images.
\Cref{fig:features:breast} shows the structures present in the breast and how these are imaginary rendered using a hand-held 2D \ac{us} probe.
\Cref{fig:features:lexicon} illustrates the lexicon proposed by the \ac{acr}~\cite{biradsus}.
\emph{And \cref{fig:features:relation}, illustrates which visual cue exploided by the lexion was targeted by the data term features proposed in Massich et al.~\cite{massich2013phd}.}
Notice that not all the visual cues used 'composing' the lexicon are suitable to build the data term.
...
Massich et al.~\cite{massich2013phd} reported that the features in \cref{fig:features:relation} extracted to describe the sample $s$ correspond to the following:
\begin{description}
  \item[Appearance] 
...
Massich et al.~\cite{massich2013phd} also reports that in order to incorporate multi-resolution, each super-pixel is group with its adjacent super-pixels such that $s' = \{s \cup \mathcal{N}_{s}\}$, the features are recalculated using $s'$ and concatenated to the original feature descriptor. This operation can be repeated several times.
...
Acoustic Posterior is mainly captured by the Brightness feature, but the Atlas also brings in crucial information to compensate for intensity inhomogeneities caused signal attenuation present in the more posterior parts of the image.The Echo Pattern cue, encodes the echoginity of a region with respect to the adipose tissue. This can be found at the anterior part of the image and its echoginity is close to the middle of the spectrum. Appearance, Brightness and the Atlas account for such information.
